<template>
  <section>
    <div class="inactive">
      <h2 class="title">Netflix 給トク払い <br class="br-sp" />新規お申込み</h2>

      <div class="title-sub gree">
        <p>＜福利厚生ご優待特典＞</p>
        <p>毎月ご利用額の<span class="red">10%相当のベネポ</span>を還元！</p>
        <!--div class="balloon3-right-btm pc">見れば見るほど<br />お得!!</div-->
      </div>
      <h3 class="title-sub">選べる2つのNetflixプラン！</h3>
      ...
      <ul class="plan_list pc">
        <li class="plan_list_item plan_list_item02">
          ...
          <button @click="toggleAccordion(7)">
            スタンダードプラン<br />お申込み
          </button>
        </li>
        <li class="plan_list_item plan_list_item03">
          ...
          <button @click="toggleAccordion(8)">
            プレミアムプラン<br />お申込み
          </button>
        </li>
      </ul>
      ...
      <table class="plan_table sp">
        <thead>
          <tr>
            <th class="plan_table_title plan_table_title02">
              スタンダードプラン
            </th>
            <th class="plan_table_title plan_table_title03">
              プレミアムプラン
            </th>
          </tr>
        </thead>
      ...
      <button class="sp button" @click="toggleAccordion(7)">お申込み</button>
      <!-- ここまでSP -->

      <!--ここからアコーディオン-->
      <div v-if="isOpen || isOpen2" class="hide">
        <div class="select">
          <h3 class="title-sub">お申込みプラン</h3>
          <div class="select-plan">
            ...
            <div>
              <input
                ...
                v-model="selectedOption"
              />
              <label for="option2" class="radio-label">プレミアムプラン</label>
            </div>
          </div>
          ...
          <div class="input-wrapper">
            ...
            <input
              ...
              :class="{
                'is-invalid': emailAgain !== '' && email !== emailAgain,
              }"
            />
          ...
          <div class="input-check_kyutoku">
            <label>
              <input type="checkbox" v-model="newsLetterOptInFlg" />
        ...
        <div class="check-kyutoku">
          <label>
            <input type="checkbox" v-model="checkKyutoku" />
            <p class="check-kyutoku_lead red">
              上記の注意事項と以下規約にご同意いただけましたら
              チェックをお願いします。
            </p>
          </label>
          <a href="https://bs.benefit-one.inc/otherPage?other_id=kiyaku_kyuyotenbiki&_ga=2.5140147.1288993255.1694393025-233433311.1690878273">給与天引き利用規約</a>
          <button class="button2" :disabled="!isValidForm" @click="apply()">
            お申込み
          </button>
          <p>
            <span class="red">ご登録手続きのため、約30分程度のお時間をいただいた後に<br />
            本ページが登録済みマイページに切り替わります。
            </span><br />
            大変ご迷惑をおかけいたしますが、ご利用プランの変更・解約などについては<br />
            約30分後を目安に再度こちらのページをご確認ください。
          </p>
        </div>
      </div>
      <!--ここまでアコーディオン-->
...
export default {
  ...
  methods: {
    toggleAccordion(selectPlanId) {
      if (selectPlanId === 7) {
        this.isOpen = !this.isOpen;
        this.isOpen2 = false;
      } else if (selectPlanId === 8) {
        this.isOpen2 = !this.isOpen2;
        this.isOpen = false;
      }
      this.selectedOption = selectPlanId;
    },
    validateEmail() {
    ...
    apply() {
      const baseUrl = process.env.VUE_APP_NETFLIX_KYUTOKU_APPLY_URL;

      const queryParams = {
        customerNumber: this.auth.user.ssoCustomerNo, //BPFお客様番号
        subscriptionServiceId: this.res.subscriptionServiceId, //SSPF側のサービスID。Netflixであれば1
        subscriptionPlanId: this.selectedOption, //SSPF側のプランID。1ベーシック2スタンダード3プレミアム
        personalEmailAddress: this.email, //個人登録メールアドレス。SSPFは任意だがフロント側で必須項目とする
        personalPhoneNumber: this.phoneNumber, //個人登録電話番号。SSPFは任意だがフロント側で必須項目とする
        newsLetterOptInFlg: this.newsLetterOptInFlg, //キャンペーンメールの受領意思。true希望するfalse希望しない
      };
