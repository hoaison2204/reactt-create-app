...
export default {
  ...
  methods: {
    handleBack() {
      ...
      this.$router.push({path: `/mypage/orders/${menuNo}?acptNo=${acptNo}&srvicDiv=${srvicDiv}&isPublicStay=${isPublicStay}`})
    },
    setBreadcrumb() {
      const { menuNo, acptNo , isPublicStay, srvicDiv } = this.$route.query
      ...
      this.$page.setBreadcrumb(
        this.$page.formatBreadcrumb("申込内容詳細", `/mypage/orders/${menuNo}?acptNo=${acptNo}&srvicDiv=${srvicDiv}&isPublicStay=${isPublicStay}`)
      )
      this.$page.setBreadcrumb(
  ...
  computed: {
    getCancelNo() {
      // state から、cancelNo を取得
      let cancelNo =  this.$store.state.orders.cancelNo
      if(!cancelNo){
        // リロード時、state の値は消える。
        // パラメータの キャンセルno を取得
        const query = this.$route.query
        cancelNo = query.cancelNo
      }
      return cancelNo
    },
    getAcptNo(){
